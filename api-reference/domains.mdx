...
<Info>
  **Modelo para integradores (una reply-domain por proyecto).** El reply domain pertenece al proyecto de la API key con la que llamas. Para ofrecer reply limpio a **tus propios clientes**, cada cliente es un proyecto RQE con su propia key: registra `reply.dominiodelcliente.com` usando la key de ese proyecto. Ejemplo: para tu cliente Phrasso, con la key del proyecto de Phrasso llamas `POST /domains/reply { "domain": "phrasso.com" }` y sus respuestas llegan a `nombre@reply.phrasso.com`. Un proyecto no puede tener más de un reply domain.
</Info>
