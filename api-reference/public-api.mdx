...
| `attachments`     | array               | No        | Lista de adjuntos (máx 10; máx 25 MB por adjunto y 25 MB en total). Estructura: `{ filename, url \| content (base64), contentType? }`. |
...
| `email_type`     | string            | No        | Naturaleza del envío: `transactional` (default) o `marketing`. Ver [Tipo de envío](/api-reference/send-email#tipo-de-envio). |
...
<Warning>
  **`email_type` ahora usa `transactional` como default** (antes era `marketing` solo en este
  endpoint, distinto del resto de la API). Si mandas campañas por `send-batch`, **declara
  `email_type: "marketing"` explícitamente**: sin el campo, el envío no respeta las bajas
  voluntarias. Los rebotes y las quejas siguen bloqueando en ambos casos.
</Warning>
...
### Gestionar un batch programado

Un batch creado con `scheduled_at` en el futuro se puede consultar, reprogramar o cancelar mientras no haya salido. Usa el `batch_id` que devolvió `POST /v1/send-batch`, con el mismo Bearer token `sk_proj_...`.

#### GET /v1/send-batch/:batchId

Devuelve el estado de un batch programado que aún no salió.
...
| `pending` | number | Jobs aún en cola con delay. `0` = el batch ya salió o se canceló. |
| `cancellable` | boolean | `true` mientras queden jobs pendientes que se puedan cancelar. |

#### PATCH /v1/send-batch/:batchId

Reprograma un batch pendiente a una nueva fecha.

| Campo | Tipo | Requerido | Descripción |
|---|---|---|---|
| `scheduled_at` | string | Si | Nueva fecha ISO 8601. Debe estar en el futuro. |
...
`rescheduled` es la cantidad de jobs reprogramados a la nueva fecha.

| Código | Descripción |
| ------ | ----------- |
| `400`  | `INVALID_INPUT` — `scheduled_at` inválido o en el pasado. |
| `404`  | `NOT_FOUND` — el batch ya salió, fue cancelado o no existe. |

#### DELETE /v1/send-batch/:batchId

Cancela un batch programado **antes** de que salga.
...
`cancelled` es la cantidad de jobs quitados de la cola; `activities_updated`, los registros de actividad marcados como `cancelled`.
...
<Warning>
  Estos endpoints solo aplican a batches con `scheduled_at` en el futuro que aún no salieron. Un batch inmediato (sin `scheduled_at`) se encola al instante y no se puede cancelar. Las activities de un batch cancelado quedan con estado `cancelled` y el motivo en `error_code`/`error_message`.
</Warning>
