...
| `data.text_body` / `data.html_body` | Cuerpo de la respuesta. Renderiza `html_body`: las imágenes embebidas (firmas, logos) ya van dentro como `data:` URIs. En `text_body` aparecen como `[cid:…]`. |
| `data.attachments[]` | Adjuntos del reply. Cada uno incluye `filename`, `content_type`, `size`, `storage_key` y `download_url` firmada con **TTL 7 días**. Descarga y persiste si necesitas retención mayor. `inline: true` marca las imágenes embebidas en el HTML (firmas, logos) — no son archivos que el remitente adjuntó; `content_id` trae su referencia `cid:`. |
