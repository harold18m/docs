...
> **Note:** Names are relative to the domain. With domain `mystore.com`, the `_amazonses` record is configured as `_amazonses.mystore.com`. Some providers append the domain automatically, so you only enter `_amazonses`.
...
> **Note:** SPF and DMARC always appear with `status: "not_set"`: their propagation is not validated automatically.
...
Looks up a domain by name and returns its basic information and sender profile.
...
| Code   | Description                                              |
| ------ | -------------------------------------------------------- |
| `400`  | `domain` parameter not provided.                         |
| `401`  | Invalid API Key.                                         |
| `404`  | Domain not found in the project.                         |
...
> **Warning:** After deleting it, you will need to register the domain and configure the DNS records again to send from it.
...
> **Note:** The `reply_email` field is saved but not included in the response.
...
> **Note:** It is faster but does not include DKIM/SPF. For better deliverability, verify the full domain.
