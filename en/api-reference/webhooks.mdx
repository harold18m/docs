...
| `data.text_body` / `data.html_body` | Body of the reply. Render `html_body`: embedded images (signatures, logos) are already inlined as `data:` URIs. In `text_body` they show up as `[cid:…]`. |
| `data.attachments[]` | Reply attachments. Each one includes `filename`, `content_type`, `size`, `storage_key`, and a `download_url` signed with a **7-day TTL**. Download and persist if you need longer retention. `inline: true` marks images embedded in the HTML (signatures, logos) — not files the sender attached; `content_id` carries their `cid:` reference. |
