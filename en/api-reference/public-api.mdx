...
> **Suppression:** as in `/v1/send-email`, applied when processing each send in the background — suppressed recipients are skipped and their activity remains with status `suppressed`.
...
> **Tip:** For more than 10,000 recipients, use multiple calls or create a campaign from the RQE Dashboard, which handles batching and retries automatically. Learn more in [Campaigns](/en/api-reference/campaigns).
