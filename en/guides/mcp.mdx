...
| `send_campaign` | **Launches a draft campaign** to its whole audience, in two steps: the first call sends nothing and returns a summary (subject, sender, template and exactly how many contacts will get the email, or what is still missing); only with `confirm: true`, after you approve that summary, does it send through the same path as the app |

<Warning>
  `send_email` and `send_campaign` (with `confirm: true`) send real email and cannot be undone. Grant `sends:execute` only to keys whose assistant should send on your behalf, and revoke it when no longer needed.
</Warning>
