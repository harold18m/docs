...
<Info>
  Your assistant can **query** data and, with write permissions, **modify** contacts and create segments. What it still **cannot** do is **send** email or create sending campaigns: that arrives in a later phase, always behind a permission you grant. No write tool sends email on its own.
</Info>
...
| `contacts:write` | Add tags and attributes to existing contacts |
...
<Warning>
  Write permissions modify your real data. None of them send email, but **adding a tag can enroll the contact into an active automation** that does send. Grant them intentionally.
</Warning>
...
<Accordion title="Writing — contacts:write / segments:write">

These modify real data. None of them send email.

| Tool | Permission | What it does |
|---|---|---|
| `add_tags` | `contacts:write` | Adds tags to an existing contact (merged, no duplicates). May enroll it into active automations with that trigger |
...
It does not send email or create sending campaigns: that needs a permission not available yet. With write permissions it can add tags and attributes to contacts and create segments, but it does not delete contacts or data. Each write capability is a separate `:write` permission you grant explicitly.
</Accordion>
